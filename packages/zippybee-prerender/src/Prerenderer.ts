...
import IRenderer, { RendererConstructor, RenderedRoute } from './IRenderer';
...
export default class Prerenderer {
  ...
  private renderer: IRenderer | any;
  ...
  public renderRoutes(routes: string[]): Promise<RenderedRoute[]> {
    return (
      this.renderer
        ...
        .then((renderedRoutes: { route: string; html: string; originalRoute: string }[]) => {
          ...
          if (this.options.postProcess) {
            ...
            return Promise.all(renderedRoutes.map((renderedRoute) => postProcess(renderedRoute, renderedRoutes))).then(() => {
              return renderedRoutes.filter(
                (route) => typeof route === 'object' && route && typeof route.html === 'string' && route.html.length > 0,
              );
            });
          }
